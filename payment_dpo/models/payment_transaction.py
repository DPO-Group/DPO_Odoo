...
class PaymentTransaction(models.Model):
    ...
    def get_dpo_token(self, values):
        ...
        payload = f"""<?xml version="1.0" encoding="utf-8"?> <API3G> <CompanyToken>{self.acquirer_id.dpo_company_token}</CompanyToken> <Request>createToken</Request> <Transaction> <PaymentAmount>{values['amount']}</PaymentAmount> <PaymentCurrency>{currency_id.name}</PaymentCurrency> <CompanyRef>{values['reference']}</CompanyRef> <RedirectURL>{return_url}</RedirectURL> <BackURL>{back_url}</BackURL> <CompanyRefUnique>0</CompanyRefUnique> <customerFirstName>{name[0]}</customerFirstName> <customerLastName>{name[-1]}</customerLastName> <customerAddress>{partner_id.street or ''}</customerAddress> <customerCity>{partner_id.city}</customerCity> <customerCountry>{partner_id.country_id.code}</customerCountry> <customerEmail>{partner_id.email or ''}</customerEmail> <customerPhone>{mobile}</customerPhone> <customerZip>{partner_id.zip or ''}</customerZip> <PTL>5</PTL> </Transaction> <Services> <Service> <ServiceType>{self.acquirer_id.dpo_service_type}</ServiceType> <ServiceDescription>{self.acquirer_id.dpo_service_description}</ServiceDescription> <ServiceDate>{datetime.strftime(datetime.now(), '%Y/%m/%d %H:%M')}</ServiceDate> </Service> </Services> <Additional> <BlockPayment>BT</BlockPayment> <BlockPayment>PP</BlockPayment> </Additional> </API3G>"""
        headers = {
            'Content-Type': 'application/xml'
        }
        url = self.acquirer_id.dpo_pay_url or 'https://secure.3gdirectpay.com/payv3.php'
        url += '/API/v6/'
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider != 'dpo':
            return res
        if not self.acquirer_id.dpo_pay_url:
            raise ValidationError("Setup DPO url first then try again.")
        api_url = self.acquirer_id.dpo_pay_url + '?ID={}'
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        tx = self.search([('reference', '=', reference), ('provider', '=', 'dpo')])
        if not tx:
    ...
    def _process_feedback_data(self, data):
        ...
        self.acquirer_reference = data.get('TransactionToken')
    ...
    def dpo_verify_token(self):
        ...
        <API3G>
          <CompanyToken>{self.acquirer_id.dpo_company_token}</CompanyToken>
